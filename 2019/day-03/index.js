...
function traceWire(path) {
  ...
  path.split(',').forEach(function(movement) {
    ...
    for(let i = 0; i < distance; i++) {
      ...
      steps += 1;
      wire.set(`${newPoint.x},${newPoint.y}`, steps);
      point.x = newPoint.x;
      point.y = newPoint.y;
    }
  })
...
function initWires() {
  ...
  let wireOne = traceWire(wires[0]);
  let wireTwo = traceWire(wires[1]);
